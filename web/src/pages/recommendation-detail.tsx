  ...
  sections: [
    {
      ...
      bullets: [
        '搜索：clawhub search "关键词" --registry https://skillhub.example.invalid',
        '查看：clawhub inspect <slug> --registry https://skillhub.example.invalid --file SKILL.md',
        '安装：clawhub install <slug> --registry https://skillhub.example.invalid',
        '逛一逛：clawhub explore --registry https://skillhub.example.invalid',
      ],
      example: {
        user: '帮我找一个能解析 PDF 的技能',
        agent: '我先在内部 SkillHub 搜索了 PDF 相关技能，找到 pdf-parsing 和 office-parsing 两个候选。pdf-parsing 支持 OCR 和扫描件，更适合你的需求。安装命令：clawhub install pdf-parsing --registry https://skillhub.example.invalid',
      },
    },
    {
      title: '发布技能前要搞清楚的事',
      ...
      example: {
        ...
        agent: '发布前需要确认你已登录。CLI 方式：先在 https://skillhub.example.invalid/login 钉钉登录，再到 Dashboard → Tokens 创建 Personal API Token，然后 clawhub login --registry https://skillhub.example.invalid --no-browser --token "<token>"。登录后用 clawhub whoami 验证，再执行发布。',
      },
    },
  ],
  media: [
